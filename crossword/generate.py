...
class CrosswordCreator():
    ...
    def consistent(self, assignment):
        ...
        used_values = set()
        for var_x, val_x in assignment.items():
            if val_x in used_values:
                return False
            used_values.add(val_x)
    ...
    def order_domain_values(self, var, assignment):
        ...
        return sorted(list(self.domains[var]), key=lambda x: vals_ruleout[x])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # Sort the variables:
        # 1. First by MRV: number of remaining values in domain (Ascending)
        # 2. Second by Degree: number of neighbors (Descending, hence the negative sign)
        unassigned.sort(key=lambda var: (
            len(self.domains[var]), -len(self.crossword.neighbors(var))))

        # Return the best candidate at the front of the sorted list
        return unassigned[0]
